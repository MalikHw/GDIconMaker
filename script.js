...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updtStts(stts) {
        ...
        checkMilestone(stts.total_visits);
    }
    
    function checkMilestone(visits) {
        const lastShown = localStorage.getItem('lastMilestoneShown') || '0';
        const milestone = Math.floor(visits / 10000) * 10000;
        
        if(milestone > 0 && milestone > parseInt(lastShown) && visits >= milestone && visits < milestone + 100) {
            showMilestoneBanner(milestone);
            localStorage.setItem('lastMilestoneShown', milestone.toString());
        }
    }
    
    function showMilestoneBanner(milestone) {
        const bar = document.getElementById('milestoneBar');
        bar.textContent = `🎉 WEEE ${(milestone/1000).toFixed(0)}K USERS! 🎉`;
